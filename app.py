...
async def receive_webhook(request: Request):
    ...
    try:
        ...
        order_match = re.search(r"\[ORDER:\s*(\d+)\s*\|\s*(\d+)\s*\|\s*(.*?)\]", reply)
        if order_match:
...
# --------------------------------------------------
# PAYSTACK PAYMENT ENDPOINTS (FASTAPI)
# --------------------------------------------------
...
async def trigger_payment(request: Request):
    ...
    try:
        ...
        res = paystack_client.trigger_mpesa_stk_push(phone_number=phone, amount=amount)
        ...
        return JSONResponse(
            content={"status": False, "message": res.get("message", "Payment initiation failed")},
            status_code=400
        )
    except Exception as e:
...
async def seller_dashboard(request: Request):
    try:
        ...
        # Use explicit keyword arguments for FastAPI/Starlette compatibility
